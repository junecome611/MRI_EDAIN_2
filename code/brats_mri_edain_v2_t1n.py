#!/usr/bin/env python
"""BraTS T1n segmentation -- MRI-EDAIN v2 trainer (blueprint section 5).

Inherits the data pipeline from the v1 BraTS script (Otsu/nonzero foreground
crop, nnU-Net augmentations, deep supervision) but swaps the v1 EDAIN layer
for the v2 input-conditional RQ-spline (mri_edain_v2.modules.MRIEDAINLayer).

Pipeline differences from v1:
    - Per-volume z-score moved upstream of the spline (NormalizeIntensityd).
    - Per-fold offline precompute (mri_edain_v2.training.precompute) supplies
      the CoordinateStandardizer fit stats and the population-Nyul theta_0.
    - Composite loss L = L_seg + lambda_anc * L_anc + lambda_kl * L_kl with the
      3-phase schedule from blueprint section 5.1.
    - EMA shadow of hypernet weights used at validation (section 5.3).

Smoke test mode:
    python brats_mri_edain_v2_t1n.py --smoke
runs precompute on 5 cases and trains 2 mini-epochs on 5/2 (train/val) to
exercise the full forward + backward + validation pathway.
"""
...
# --- v2 components ---------------------------------------------------------
# Ensure the project root is importable regardless of where the script is run.
...
# =============================================================================
# Path configuration -- defaults assume the repo and the dataset dir are
# sibling directories: <parent>/MRI_EDAIN_2 and <parent>/dataset/brats.
# Override with --data_dir / --out_dir at the command line.
# =============================================================================

_DEFAULT_DATA_DIR = (Path(__file__).resolve().parent.parent.parent
                     / "dataset" / "brats")

# These globals get overwritten in main() once --data_dir is parsed.
DATA_DIR: Path = _DEFAULT_DATA_DIR
T1N_DIR: Path = DATA_DIR / "brats_t1n"
SPLIT_JSON: Path = DATA_DIR / "split.json"
OUT_DIR: Path = Path("./outputs/brats_mri_edain_v2")
LOG_DIR: Path = Path("./logs/brats_mri_edain_v2")
ARTIFACT_DIR: Path = Path("./artifacts/brats_mri_edain_v2")
...
# Loss schedule defaults (blueprint section 5.1; total_steps overridden per fold).
LAMBDA_ANC_INIT = 1.0e-2
LAMBDA_ANC_FINAL = 1.0e-4
LAMBDA_KL_FINAL = 1.0e-4

# EMA decay (blueprint section 5.3).
EMA_DECAY = 0.99

# Grad clip on hypernet (blueprint section 5.2 "CRITICAL").
HYPERNET_GRAD_CLIP = 1.0


# =============================================================================
# nnU-Net style augmentations (copied verbatim from brats_edain_zscore_1.py
# so this file stays self-contained for cluster jobs).
# =============================================================================
...
class RemapLabelsd(MapTransform):
    """Remap BraTS labels: any positive label -> 1 (tumor), 0 -> 0."""
...
class SegmentationModelWithMRIEDAINv2(nn.Module):
    """DynUNet backbone preceded by the v2 input-conditional spline layer.

    The spline operates on already-z-scored input (NormalizeIntensityd upstream)
    and computes gamma on whatever it receives (post-augmentation patch at
    training time; full volume passed in via the gamma_raw kwarg at inference).
    """
...
def build_file_entry(case_id):
    return {
        "image":   str(T1N_DIR / f"{case_id}-t1n.nii.gz"),
        "label":   str(T1N_DIR / f"{case_id}-seg.nii.gz"),
        "case_id": case_id,
    }
...
def load_data_and_compute_fingerprint(smoke: bool = False,
    ...
    folds = []
    for fold_key in [f"fold_{i}" for i in range(5)]:
        train = split_data[fold_key]["train"]
        val = split_data[fold_key]["val"]
        if smoke:
            train = train[:5]
            val = val[:2]
        folds.append({
            "fold": int(fold_key.split("_")[1]),
            "train_files": [build_file_entry(c) for c in train],
            "val_files":   [build_file_entry(c) for c in val],
        })

    sample_ids = (folds[0]["train_files"][:5] if smoke
                  else [{"image": f"{T1N_DIR}/{c}-t1n.nii.gz"}
                        for c in split_data["fold_0"]["train"][:100]])
    if not smoke:
        sample_ids = [build_file_entry(c) for c in split_data["fold_0"]["train"][:100]]
...
def train_fold(fold_info, fp, device, *, smoke: bool = False, max_epochs: int = None,
    ...
    print(f"\n{'='*60}")
    print(f"Fold {CURR_FOLD} (MRI-EDAIN v2) | "
    ...
    # -------------------------------------------------------------------------
    # Precompute artifact: standardizer fit + theta_0
    # -------------------------------------------------------------------------
    ...
    print(f"[precompute] population landmarks (z-scored intensity scale): "
          f"{artifact.population_landmarks.tolist()}")

    # -------------------------------------------------------------------------
    # Build model (DynUNet + MRIEDAINLayer)
    # -------------------------------------------------------------------------
    ...
    backbone_params = total_params - edain_params
    print(f"Backbone params: {backbone_params / 1e6:.2f} M | "
          f"EDAINv2 params (hypernet only): {edain_params}")
    ...
    # -------------------------------------------------------------------------
    # Optimizer, losses, scheduler, EMA
    # -------------------------------------------------------------------------
    ...
    lambda_sched = LambdaScheduler(
        ...
        lambda_anc_init=LAMBDA_ANC_INIT,
        lambda_anc_final=LAMBDA_ANC_FINAL,
    ...
    for epoch in range(1, epochs_target + 1):
        ...
        for batch in train_loader:
            ...
            # Phase 0: freeze hypernet (set requires_grad=False); unfreeze later.
            ...
            with amp.autocast(device_type="cuda", enabled=torch.cuda.is_available()):
                # Forward: spline applied internally inside the model.
                ...
                # Anchor loss: re-derive current spline params from per-batch
                # theta (cheap; the diag already contains gamma_std).
                ...
                # KL loss (skipped if weight is zero, saves memory in early phases).
                if kl_w > 0:
        ...
        if epoch % VAL_INTERVAL == 0 or epoch == epochs_target:
            ...
            with torch.inference_mode(), \
                ...
                for v_i, val_data in enumerate(val_loader):
                    ...
                    # Whole-volume gamma so sliding-window patches share spline.
                    ...
                    def predictor(x):
                        x_tilde, _ = model.edain(x, mask=(x != 0.0),
                                                  gamma_raw=gamma_vol.unsqueeze(0).expand(x.shape[0], -1),
                                                  return_diagnostics=False)
                        out = model.backbone(x_tilde)
                    ...
                    # Phase-I diagnostic: non-affineness r_i for this case.
                    gamma_std_vol = standardizer(gamma_vol.unsqueeze(0))
                    delta_vol = model.edain.hypernet(gamma_std_vol)
                    theta_vol = model.edain.theta_0 + delta_vol  # (1, 3K-1)
...
def main():
    ...
    parser.add_argument(
        ...
        help=f"Path to BraTS data dir containing brats_t1n/ and split.json. "
    ...
    parser.add_argument(
        ...
        help="Sliding-window batch size at validation (default 1). Each "
    ...
    if not T1N_DIR.exists():
        raise FileNotFoundError(
            f"BraTS T1n dir not found: {T1N_DIR}\n"
            f"Pass --data_dir <path-to-brats-root> where the root contains "
            f"`brats_t1n/` and `split.json`."
        )
    ...
    print("=" * 70)
    print("BRATS T1n  -  MRI-EDAIN v2 Adaptive Normalization")
    print("=" * 70)
    print(f"T1n dir   : {T1N_DIR}")
